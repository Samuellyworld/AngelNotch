...
enum ArcFaceEmbedderError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .modelNotFound:
      return "ArcFace.mlpackage/mlmodelc was not found in AngelNotch."
    ...
    case .unexpectedInputSize(let got, let expected):
      return
        "ArcFaceEmbedder expects a \(expected)x\(expected) aligned image, got \(got.0)x\(got.1). Run the face through FaceAligner first."
  ...
  private static func locateModel() -> URL? {
    for name in ["ArcFace", "w600k_mbf"] {
      if let url = Bundle.main.url(
        ...
        subdirectory: "faceunlock"
      ) {
        return url
      }
      let bundledPackage = Bundle.main.url(
        forResource: name,
        withExtension: "mlpackage",
        subdirectory: "faceunlock"
      )
      let developmentPackage = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appending(path: "app/sources/angelnotch/resources/faceunlock/\(name).mlpackage")
      let packageURL =
        bundledPackage
        ?? (FileManager.default.fileExists(atPath: developmentPackage.path)
          ? developmentPackage : nil)
      if let packageURL,
  ...
  nonisolated func embedding(for face: CGImage) throws -> [Float] {
    guard face.width == Self.inputSize, face.height == Self.inputSize else {
      throw ArcFaceEmbedderError.unexpectedInputSize(
        got: (face.width, face.height), expected: Self.inputSize)
    }
